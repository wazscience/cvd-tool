 ...
 * 
 * Usage: node consolidate-css.mjs
 * 
 * Note: This file uses .mjs extension to explicitly use ES modules
 * without changing your package.json "type" setting.
 */
...
// Try to load chalk, and if it fails, use our simple colors implementation
let chalk;
try {
  chalk = require('chalk');
  // Test if chalk is fully functional
  if (typeof chalk.blue !== 'function') {
    chalk = colors;
  }
} catch (error) {
  // Use our simple colors implementation
  chalk = colors;
}
...
async function main() {
  ...
  try {
    ...
    if (success) {
      ...
      console.log(`All styles have been consolidated into a single CSS file referenced by index.html.`);
      console.log(`Backups of the original files are available in: ${BACKUP_DIR}`);
    } else {
